...
$app->get('/usuarios/', function() use ($app) {
    ...
    if($auth->isAuth($authToken)){
       ...
       /* $UserPoints = $app->request->params('userPoints');
        ...
        if (isset($byUser)){ 
            $usuarios = array('usuarios' => $dbUsuario->obtenerUsuario("",0));
        }  else {
            $usuarios = array('usuarios' => $dbUsuario->obtenerUsuario($byUser,2));
        }
...
$app->get('/usuarios/:id', function($id) use ($app) {
   /* $auth = new Auth();
    ...
    if($auth->isAuth($authToken)){*/
        $dbUsuario = new DbUsuario(); 
        $resultUsuario = $dbUsuario->obtenerUsuario($id,1);
        $app->response->headers->set('Content-Type', 'application/json');
        $app->response->setStatus(200);
        $app->response->setBody($resultUsuario->toJson());
   /* }
